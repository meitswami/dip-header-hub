...
- **External MySQL Connections** — Admins can register read-only MySQL databases (at `/admin/mysql`), browse their schema, sample tables, and run SELECT/SHOW queries. Passwords are encrypted at rest with Fernet.

### AI / Retrieval (Phase 1 + 2)
- **Universal Document Ingestion** — Per-case and global knowledge base supports PDF, DOCX, PPTX, XLSX/XLS, CSV/TSV, TXT/MD, SQL dumps, and images (PNG/JPG/WEBP/TIFF) via Tesseract OCR (English + Hindi)
- **Local Multilingual Embeddings** — `paraphrase-multilingual-MiniLM-L12-v2` (Hindi + English, no cloud)
- **Hybrid Retrieval** — Structured CDR facts + semantic vector search + entity-bridge (exact match on phone/IMEI/IP numbers from the question)
- **Optional Cross-Encoder Reranker** — `ms-marco-MiniLM-L-6-v2` rescorer, enable with `DIP_USE_RERANKER=1`
- **Grounded Chat with Citations** — Streaming answers (SSE) quoting `[n]` against the exact source file + page/row/sheet. Fast (3B) and Accurate (7B) tiers toggle in the chat header
- **Column Explainer** — Click the info icon on any record column header (CDR / IPDR / SDR / Tower) to see the investigator-friendly definition (Hindi + English), abbreviations, and 3 sample values from the current case

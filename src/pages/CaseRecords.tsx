...
import {
  ArrowLeft, FileSpreadsheet, Search, Download, Loader2, ChevronLeft, X, Trash2, Upload,
} from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { parseSpreadsheet, ParseResult } from '@/lib/dataParser';
...
export default function CaseRecords() {
  ...
  const openFile = async (file: any) => {
    ...
    try {
      ...
      // Download from storage using the URL directly
  ...
  const deleteFile = async (file: any) => {
    ...
    try {
      ...
      const urlParts = file.file_url.split('/evidence/');
      if (urlParts[1]) {
        await supabase.storage.from('evidence').remove([decodeURIComponent(urlParts[1])]);
      }
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {headers.map(h => (
                        <TableHead key={h} className="whitespace-nowrap text-xs sticky top-0 bg-background">{h}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
